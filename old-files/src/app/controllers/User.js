...
export async function sendEmail(email, id, res) {
    ...
    let linkTag = `<a href="http://localhost:6001/atualizar/senha?token=${token}"> Redefinir senha</a>`;
    let msg = `Por favor clique no link a seguir para redefinir sua senha <br><br>${linkTag}`;
...
class UserController {
    async index(req, res) {
        try {
            ...
            let users;
            let findusersWhioutPhysicalPerson;

            if (!findUsers.physical_person) {
                findusersWhioutPhysicalPerson = await User.findAll({
                    where: { active: true },
                    include: [
                        {
                            model: Unit,
            ...
            const mergeListsUsers = users.concat(findusersWhioutPhysicalPerson);

            var listAllUsers = new Map();

            mergeListsUsers.forEach(item => {
                if (!listAllUsers.has(item.id)) {
                    listAllUsers.set(item.id, item);
                }
            });

            return res.json(content([...listAllUsers.values()]));
        } catch (e) {
            console.error(e);
        }
    }
            ...
            if (data.menus) {
                await Promise.all(
                    data.menus.map(async element => {
                        await Promise.all(
                            element.children.map(async children => {
                                // children.permission_read = 1
                                if (children.url != '/users') {
                                    if (
                                        children.permission_read ||
                                        children.permission_write ||
                                        children.permission_delete
                                    ) {
                                        let user_menu = {
                                            menu: children.menu,
                                            user: user_stored.id,
    ...
    async update(req, res) {
        ...
        try {
            ...
            let address_updated = await Address.update(data.address, {
                where: { id: data.pf.Person.address },
                transaction
            });

            let contact_updated = await Contact.update(data.contact, {
                where: { id: data.pf.Person.contact },
                transaction
            });
            ...
            let physical_person_updated = await PhysicalPerson.update(
                physical_person_obj,
                { where: { id: data.pf.id }, transaction }
            );
            ...
            let user_updated = await User.update(user_obj, {
                where: { id: data.id },
                transaction
            });
                        ...
                        await Promise.all(
                            element.children.map(async children => {
                                ...
                                if (!children.id) {
                                    ...
                                    await UserMenu.update(user_menu, {
    ...
    async reenviarSenha(req, res) {
        ...
        try {
            ...
            if (!user) {
                return res.status(400).json({
                    error:
                        'Erro ao carregar usuario, favor entrar em contato com nosso suporte'
                });
            }
            ...
            return res.json(
                content(
                    'Email para definição de senha enviado com sucesso para ',
                    email
                )
            );
        } catch (error) {
            ...
            try {
                ...
                await fs.readFile(
                    ...
                    function(err, contents) {
                        ...
                        let msgContent = contents.replace(
                            '[CONTENT VALUE]',
                            msg
                        );
                ...
                return res.status(400).json({
                    error:
                        'Erro ao cadastrar senha, entre em contato com nosso suporte!'
                });
            }
            // return "";
    ...
    async atualizarSenha(req, res) {
        ...
        try {
            ...
            return res.json(
                content('senha atualizada com sucesso', user_updated)
            );
        } catch (err) {
