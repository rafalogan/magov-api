...
export class UserLogService {
	...
	async read(options: ReadOptionsModel) {
		try {
			const page = options.page || 1;
			const limit = options.limit || 10;

			const total = (await this.count(Number(options.tenancyId))) as number;
			const fromDB = await this.conn({ ul: 'users_logs', u: 'users' })
				.select(
				...
				.where('ul.tenancy_id', options.tenancyId)
			...
			for (const item of fromDB) {
				data.push(new UserLogView({ ...convertDataValues(item) }));
			}
